...
export async function POST(): Promise<NextResponse> {
    ...
    if (platform === 'linux') {
        // Wayland/Kitty prioritization + common fallbacks + ubuntu default
        command = `x-terminal-emulator -e "node ${scriptPath}" || kitty -- node ${scriptPath} || gnome-terminal -- node ${scriptPath} || konsole -e node ${scriptPath} || xfce4-terminal -e "node ${scriptPath}" || xterm -e "node ${scriptPath}"`;
